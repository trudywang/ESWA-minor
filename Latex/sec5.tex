...
%For Aalborg, there are 2373 and 2373 queries in training and test sets, respectively; for Beijing, there are 6464 and 6464 queries in training and test sets, respectively; for Singapore, we draw 15,000 trajectories from a taxi trajectory data of Singapore randomly, and then we we extract each trajectory's start point and end point, and get the nodes number corresponding to the nodes on the road network, the start node and end node will be the source and target of a shortest path request.We get 15000 queries to be the query log at last, half queries are seen as the training set and remnant 50\% queries as the test set.
%
